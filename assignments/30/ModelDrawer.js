function ModelDrawer(model, shaderProgram)
{
   ...
   this.draw = function(MV)
      {
         ...
         if (this.transTexture != null)
         {
            shaderProgram.GL.activeTexture(webGL.GL.TEXTURE3);
            shaderProgram.GL.bindTexture(shaderProgram.GL.TEXTURE_2D, this.transTexture);
            shaderProgram.GL.uniform1f(this.shaderProgram.baseTrans_pos, 0.0);
            this.transTexture = null;
         }
   ...
   this.setTransImage = function(image, webGL)
      {
         this.transTexture = webGL.LoadTexture(image);
      }
}
